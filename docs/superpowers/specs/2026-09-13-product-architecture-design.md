...
- The control plane records the approval (approver's GitHub user id and login, repository id, file path, the content hash today's stamps already carry, time) and writes the stamp commit through the App.
...
- The server's elevated database key is used only for event ingest, webhooks and the reconciler that repairs run state from GitHub's API; ingest and webhooks verify their caller first, and the reconciler takes no external input.
