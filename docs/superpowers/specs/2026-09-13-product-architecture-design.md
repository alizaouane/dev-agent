...
- **Dispatch and attempt tests:** a dispatch whose response is lost binds to the started run by `run-name` without a duplicate, whether the reconciler or the run's own pre-flight call binds first, and two concurrent binders produce one binding; a re-run's higher attempt moves a failed run back to `running` and its outcome becomes the run's status.
