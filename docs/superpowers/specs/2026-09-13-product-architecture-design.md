...
- **Reservation idempotency test:** a product-started first attempt's pre-flight call returns the reservation made before dispatch without reserving again, repeated calls for one attempt return one reservation, and a re-run attempt reserves anew.
