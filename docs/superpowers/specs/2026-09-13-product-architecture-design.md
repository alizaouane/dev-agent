...
- **GitHub Actions (batch runs).** The GitHub App calls `workflow_dispatch` on the consumer's wrapper with the product run id as an input. GitHub returns the created workflow run id; the control plane stores it before the run leaves `starting`. **An ambiguous dispatch is not a failure:** if the dispatch call errors or times out, GitHub may still have started the workflow. The wrapper sets its `run-name` to the product run id, so the run stays `starting`, keeps its reservation and refuses a retry while the reconciler looks for a workflow run with that name created after the dispatch. A match binds the run; no match within the start timeout fails it as never started and releases the reservation. The workflow can reach its first authenticated call (the pre-flight reservation or an event) before the reconciler runs, so that call can also bind the run, atomically (section 5.3 step 3). Only a definite rejection from GitHub (for example 404 or 422) fails the run at once.
- **Re-runs are attempts of the same run.** GitHub keeps the workflow run id and increments `run_attempt` when a run is re-run. Each attempt is recorded separately under the product run, with its own status and events; a new, higher attempt moves the product run back to `running`, and the product run's status is always that of its latest attempt. **Every attempt reserves before it can spend, wherever it was started.** A re-run started from GitHub is already executing by the time a webhook arrives, so the product cannot reserve for it from outside. Instead the engine's pre-flight budget step (today's `budget-gate.ts`), which runs before any billable step in every phase job, asks the control plane for a reservation for its workflow run id and attempt, authenticated with the job's OIDC token. The control plane creates the run (adopted, as in section 5.3 step 3) and the attempt if it has not seen them. The call is idempotent per run and attempt: when a reservation already exists for that attempt, including the one a product-started run made before dispatch for its first attempt, it returns that reservation and reserves nothing more. It reserves anew, in the same locked transaction as any other run, only for an adopted run's attempt or a higher attempt without one. **The per-run cap covers all attempts of a run together:** a higher attempt reserves only the run's remaining allowance, the run cap minus what its earlier attempts actually spent (their unused reservations having been released), and the call refuses if that remainder is zero or does not fit the project and organisation budgets; a refusal or an unreachable control plane fails the step before the agent starts. Part 3 builds the reservation endpoint and switches the engine's pre-flight step to it in the same release; until then, today's budget gate stays in place, so part 2 ships with every phase still runnable. The wrapper runs the engine at a version pinned per project (a tag or SHA, in both the wrapper `uses:` ref and the engine checkout), replacing today's `@main`.
- **Binding comes before adoption, on every path.** Three things can be first to learn about a workflow run: its `workflow_run` webhook, the reconciler, and the run's first authenticated call. All three go through one routine: look for an unbound product run in `starting` for the same project whose id equals the workflow run's `run-name`, and bind it with the conditional update of section 5.3 step 3. Only when no such run exists is an adopted run created. So a lost dispatch response can never produce a second run for the same workflow run, whichever of the three arrives first.
- **Runs the product did not start.** Wrappers keep their event triggers during migration. A workflow run that binds to no product run becomes an adopted run (origin `github`), so every run appears in the product. Part 5 decides per repository whether those triggers stay.
...
3. The engine posts events authenticated with the job's GitHub Actions OIDC token. The control plane requires all of: a product-specific `aud`; `repository_id` equal to the project's repository id (not the name, which can change); `job_workflow_ref` equal to the pinned engine workflow for that project (until part 2 pins an engine version per project, one of the dev-agent workflow refs the project's wrapper calls); and `run_id` equal to the stored workflow run id. **Binding an unbound run:** if the request names a product run (from the wrapper's input) that is still `starting` with no stored workflow run id, the control plane reads that workflow run from the GitHub API, checks that its `run-name` equals the product run id and its repository matches, and stores the token's `run_id` with a conditional update that succeeds only while the stored id is still empty. Whether that update wins or loses, the control plane then re-reads the stored id and applies the normal equality check: a request whose `run_id` equals the stored id is accepted, including one that lost a race to a binder carrying the same id, and only a different id or a mismatched `run-name` is refused. For a run the product did not start, the first authenticated call with an otherwise valid token, whether the pre-flight reservation or an event, first tries to bind as above (section 4.3, binding comes before adoption) and otherwise creates the adopted run and its attempt from the token's claims. The insert is unique on the workflow run id, so concurrent first calls and the later `workflow_run` webhook all converge on the same run, and neither a reservation nor an event arriving before the webhook is rejected.
...
- **Levels:** organisation per month, project per month, per run (across all of a run's attempts), per chat session. Alerts at 50%, 80% and 100%.
...
| Run re-run from GitHub | The attempt's pre-flight step obtains its own reservation before any billable step; the product records the attempt and the run returns to `running`. Without a reservation the step fails and nothing is spent. | Attempt selector on the run page |
...
- **Reservation idempotency test:** a product-started first attempt's pre-flight call returns the reservation made before dispatch without reserving again, repeated calls for one attempt return one reservation, and a re-run attempt reserves only the run cap minus what earlier attempts spent, refusing when nothing remains.
- **Dispatch and attempt tests:** a dispatch whose response is lost binds to the started run by `run-name` without a duplicate, whether the webhook, the reconciler or the run's own pre-flight call arrives first, with no adopted duplicate, and two concurrent binders carrying the same `run_id` produce one binding with both requests accepted, while a binder with a different `run_id` is refused; a re-run's higher attempt moves a failed run back to `running` and its outcome becomes the run's status.
