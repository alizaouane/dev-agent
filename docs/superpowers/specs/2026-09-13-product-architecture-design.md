...
3. The engine posts events authenticated with the job's GitHub Actions OIDC token. The control plane requires all of: a product-specific `aud`; `repository_id` equal to the project's repository id (not the name, which can change); `job_workflow_ref` equal to the pinned engine workflow for that project (until part 2 pins an engine version per project, one of the dev-agent workflow refs the project's wrapper calls). It then resolves the token's workflow run to a product run with the routine of section 5.6, and the token's `run_id` and `run_attempt` must belong to that run. A call is refused when its own attempt is terminal, or when the run ended as never started, cancelled late, or a duplicate dispatch; a higher attempt on a finished run is a re-run and is accepted.
