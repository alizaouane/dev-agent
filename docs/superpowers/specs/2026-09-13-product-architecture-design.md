...
- **Re-runs are attempts of the same run.** GitHub keeps the workflow run id and increments `run_attempt` when a run is re-run. Each attempt is recorded separately under the product run, with its own status and events; a new, higher attempt moves the product run back to `running`, and the product run's status is always that of its latest attempt. **Every attempt reserves before it can spend, wherever it was started.** A re-run started from GitHub is already executing by the time a webhook arrives, so the product cannot reserve for it from outside. Instead the engine's pre-flight budget step (today's `budget-gate.ts`), which runs before any billable step in every phase job, asks the control plane for a reservation for its workflow run id and attempt, authenticated with the job's OIDC token. The control plane creates the run (adopted, as in section 5.3 step 3) and the attempt if it has not seen them, reserves in the same locked transaction as any other run, and refuses if the reservation does not fit; a refusal or an unreachable control plane fails the step before the agent starts. Part 3 builds the reservation endpoint and switches the engine's pre-flight step to it in the same release; until then, today's budget gate stays in place, so part 2 ships with every phase still runnable. The wrapper runs the engine at a version pinned per project (a tag or SHA, in both the wrapper `uses:` ref and the engine checkout), replacing today's `@main`.
