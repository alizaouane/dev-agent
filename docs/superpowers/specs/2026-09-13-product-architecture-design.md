...
3. The engine posts events authenticated with the job's GitHub Actions OIDC token. The control plane requires all of: a product-specific `aud`; `repository_id` equal to the project's repository id (not the name, which can change); `job_workflow_ref` equal to the pinned engine workflow for that project (until part 2 pins an engine version per project, one of the dev-agent workflow refs the project's wrapper calls). It then resolves the token's workflow run to a product run with the routine of section 5.6, and the token's `run_id` and `run_attempt` must belong to that run. Calls from a never-started or late-cancelled run, or from a workflow run that is a duplicate dispatch, are refused. A pre-flight call from an attempt that is already terminal is refused; an event post from such an attempt is stored without changing its status, since a cancelled job may still send its last events. A higher attempt on a finished run is a re-run and is accepted.
4. **Status never comes from events.** Events are display only, and a run's status comes from GitHub's workflow result.
5. **Gate verdicts come only from the gate job's own result.** Today ACM and swarm verdicts are labels and comments written with the token of the job the agent runs in, so re-reading them would trust data produced next to the agent. Check runs cannot fix this: every check run created in Actions is credited to the same app, and nothing in the API ties one to the job that created it. In the product design, each gate runs in its own job with no agent step, and that job's final verdict step fails exactly when the gate says no. The product reads the verdict from `GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs` for the stored workflow run id and attempt, matching the gate job by its name in the pinned engine workflow. The verdict step's conclusion decides pass or fail; a failure in any earlier step of that job counts as `infrastructure`, not a gate result, except the pre-flight step (section 5.6), whose refusal maps to `budget`, or to cancelled late or never started for such runs, as the refusal reason says. Verdict details travel as an artifact and are display only. Approvals are verified from signed stamps (section 4.2).
...
**Resolution routine.** The `workflow_run` webhook, the reconciler and every OIDC-authenticated call (events and the pre-flight reservation) resolve a workflow run through this one routine, under a row lock on the candidate product run. For OIDC calls, a token whose `run_id` and `run_attempt` already resolve to a stored binding and attempt skips the GitHub read, so a GitHub outage does not block events for runs already bound. Otherwise the workflow run's fields are read from the GitHub API for the token's `run_id`, and the token's `event_name` must match them.
...
- The engine's pre-flight step runs in every job of a phase workflow as the first step after checking out the engine and setting up its runtime, which have no side effects and read no secrets, and before any step that can spend, push or comment. A late, duplicate or never-started run is therefore refused before it has side effects. It calls the reservation endpoint with its OIDC token. The call resolves the run with the routine above and creates the attempt row if it is new.
- The call is idempotent per run and attempt: an active reservation for that attempt, including the one made before dispatch for attempt 1, is returned and nothing more is reserved. It is refused when the calling attempt is terminal, when the run ended as never started or cancelled late, or when the workflow run is a duplicate dispatch. A higher attempt on a finished run is a re-run: the call creates that attempt and reserves for it, even if its `workflow_run` webhook has not arrived and the run still shows its previous terminal status.
...
- **Delivery order.** Part 2 ships the pre-flight call with resolution and the refusal of late, duplicate, never-started and terminal-attempt calls, without reserving, next to today's budget gate; so no run the product dispatches is unguarded. Part 3 adds the reservation to that same call and retires today's budget gate in the same release, so every phase stays runnable after each part.
...
| Run re-run from GitHub | A new attempt: its pre-flight step settles earlier attempts and reserves the run's remaining cap before any step with side effects (section 5.6), even while the run still shows its previous terminal status; without a reservation the step fails and nothing is spent | Attempt selector on the run page |
...
- **Reservation tests:** attempt 1 of a product run reuses its pre-dispatch reservation; repeated pre-flight calls for one attempt return one reservation; a re-run attempt reserves only the run cap minus earlier actual spend and refuses when nothing remains; a re-run whose earlier attempt's completion webhook was lost settles that attempt first and is not wrongly refused; a re-run attempt resolves a failed run back to `running`, and its pre-flight call is accepted and reserves even before the re-run's webhook arrives; a pre-flight refusal in a gate job is reported as `budget` or late, not `infrastructure`; an already-bound run's events are accepted while GitHub is unreadable.
...
2. **Agent adapter in the engine:** replace `claude-code-action` with the adapter, Claude Code first; OIDC-authenticated events; engine pinned per project; `verify-approval` accepts only signed product stamps on moved projects; gate jobs separated from agent jobs, with verdicts read from job conclusions; credential proxy or sidecar poster; narrowed Bash allowlists; stub adapter; the pre-flight call with run resolution and late, duplicate and terminal refusal (section 5.6).
3. **Credentials and cost:** per-organisation keys and KMS, secret sync, reservations added to the pre-flight call and today's budget gate retired, reserve-then-cancel, price table, console limit confirmation; the legal review of vendor terms gates it.
