...
The wrapper's spec-path regex is intentionally permissive. A stray `docs/specs/old.md` reference in a code block, a deprecation note, or a "see also" aside could match before the canonical spec link if it appears earlier in the issue body. If that happens, the probe will be authored from the wrong spec's acceptance criteria. To fix it, edit the issue body so the canonical spec link appears before any stray references, or wrap the stray references in backtick code spans (the regex operates on the raw body text, so backtick-quoted paths still match — use an HTML comment `<!-- docs/specs/old.md -->` to suppress matching instead). A follow-up to add a structured `Spec:` field convention to the issue template would eliminate the ambiguity.
