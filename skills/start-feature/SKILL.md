...
6. **Optional elicit pass per major section.** After drafting `## Context`, `## Acceptance Criteria`, `## Architecture`, or `## Edge cases`, you may offer one round of `dev-agent:elicit` on that section. Invoke via the `Skill` tool with `section_name=<the H2 header>` and `section_content=<the just-drafted text>`. The skill loops on its menu and returns the enhanced section text on the user's `x`. Replace the draft with the returned value. The skill cannot advance Phase 2 on its own — when it returns, you're back here writing the next section. Don't offer elicit for every section; pick the 1-2 where the first draft feels thin.
