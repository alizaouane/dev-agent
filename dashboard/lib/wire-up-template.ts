...
# Node version dev-agent's gates use for this repo's own install, tests and
# typecheck. Leave unset to use the repo's .nvmrc, .node-version,
# .tool-versions or package.json engines; with none of those, the engine's
# Node applies.
# runtime:
#   node: "22"
...
# One run per issue and phase at a time. The dashboard checks for runs in
# flight before it dispatches, but a check followed by a dispatch is not
# atomic: two clicks landing together can both see nothing running and both
# enqueue. GitHub serialises on this key, so the second waits for the first
# instead of a second agent working the same branch.
#
# cancel-in-progress stays false deliberately — an implement run that is
# midway through committing should finish, not be killed by a duplicate.
concurrency:
  group: dev-agent-\${{ inputs.phase }}-\${{ inputs.issue_number }}
...
permissions:
  contents: read
  pull-requests: write
  # \`statusCheckRollup\` reads CheckRun and StatusContext nodes, and the token
  # cannot see either without these. Without them \`gh pr list\` fails outright
  # with "Resource not accessible by integration" — the sweep does not degrade
  # to seeing no checks, it errors, so every PR goes untriaged.
  checks: read
  statuses: read
